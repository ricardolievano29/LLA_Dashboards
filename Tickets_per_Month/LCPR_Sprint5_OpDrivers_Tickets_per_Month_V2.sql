...
SELECT
    *,
    ...
    --- ### Option 2: run_timestamp
    -- cast(substr(cast(run_timestamp as varchar), 1, 10) as date) as interaction_date, 
    -- date_trunc('month', cast(substr(cast(run_timestamp as varchar), 1, 10) as date)) as month
...
SELECT
    ...
    --- ### Option 2: Non-distinct interaction_date (defined previously)
...
-- SELECT fix_s_att_account ,sum(number_tickets) FROM final_fields GROUP BY 1 ORDER BY 2 desc
