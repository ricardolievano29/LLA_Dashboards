------------------------ WARNING: (2023/04/22) The code is not running due to a problem with the column names in service_delivery.
...
, clean_interaction_time as (SELECT * FROM "lcpr.stage.prod"."lcpr_interactions_csg" WHERE cast(interaction_start_time as varchar) != ' ' and interaction_start_time is not null and date_trunc('month', date(interaction_start_time)) = (SELECT input_month FROM parameters) and account_type = 'RES')
...
, invol_churners as (SELECT * FROM "lla_cco_lcpr_ana_prod"."lcpr_fmc_churn_dev" WHERE fmc_s_fla_churntype = 'Involuntary Churner')

, join_interactions as (SELECT a.*, b.*, case when interaction_type in ('tech_call', 'truckroll') then interaction_id else null end as ticket_flag FROM full_interactions a LEFT JOIN customers_dna b ON cast(a.account_id as varchar) = cast(b.sub_acct_no_sbb as varchar))

, interactions_panel as (SELECT interaction_id, interaction_date, interaction_type, account_id, bridger_addr_hse, count(distinct ticket_flag) as tech_tickets FROM join_interactions GROUP BY 1, 2, 3, 4, 5)

, node_panel as (SELECT bridger_addr_hse, tech_tickets FROM interactions_panel ORDER BY 2 desc)

, summary_tickets as (SELECT * FROM interactions_panel)

, final_pre1 as (SELECT bridger_addr_hse, count(distinct n.sub_acct_no_sbb) as total_accounts, count(distinct account_id_vol_dx) as vol_churners, count(distinct i.fmc_s_att_account) as invol_churners FROM customers_dna n LEFT JOIN vol_churners v
    ON cast(n.sub_acct_no_sbb as varchar) = cast(v.account_id_vol_dx as varchar) LEFT JOIN invol_churners i ON cast(n.sub_acct_no_sbb as varchar) = cast(i.fmc_s_att_account as varchar) GROUP BY 1)

, final_pre2 as (SELECT bridger_addr_hse, count(distinct account_id) as unique_act_with_tickets, sum(tech_tickets) as num_tickets FROM summary_tickets GROUP BY bridger_addr_hse)

, final as (SELECT *, unique_act_with_tickets, num_tickets, cast(unique_act_with_tickets as double)/cast(total_accounts as double) as act_with_ticket_rate FROM final_pre1 a LEFT JOIN final_pre2 b ON a.bridger_addr_hse = b.bridger_addr_hse WHERE a.total_accounts > 30)
