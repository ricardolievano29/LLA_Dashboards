...
, account_panel as (SELECT distinct account_id, bridger_addr_hse as node_id, max(ticket_flag) as ticket_flag FROM join_interactions GROUP BY 1, 2)

, nodes_panel as (SELECT distinct node_id, count(distinct account_id) as cust_in_node, count(distinct case when ticket_flag is not null then account_id else null end) as cust_w_tickets_in_node FROM account_panel GROUP BY 1)

, final as (SELECT node_id, cust_in_node, cust_w_tickets_in_node, case when cast(cust_w_tickets_in_node as double)/cast(cust_in_node as double) > 0.06 then node_id else null end as nodes_ticket_density_above_6pct FROM nodes_panel)

, nodes_result as (SELECT 'LCPR' as Opco, cast(count(distinct nodes_ticket_density_above_6pct) as double)/cast(count(distinct node_id) as double) as nodes_ticket_density_6pct FROM final)
